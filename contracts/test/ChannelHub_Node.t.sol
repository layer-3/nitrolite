...
contract ChannelHubTest_withdrawFromNode is ChannelHubTest_Base {
    ...
    function test_emitsEvents_withERC20() public {
        ...
        emit ChannelHub.Withdrawn(address(token), DEPOSIT_AMOUNT);
    ...
    function test_succeeds_fullBalance_ERC20() public {
        vm.prank(node);
        cHub.withdrawFromNode(recipient, address(token), INITIAL_BALANCE);

        assertEq(token.balanceOf(recipient), INITIAL_BALANCE, "Recipient should receive full balance");
        assertEq(cHub.getNodeBalance(address(token)), 0, "Node vault balance should be zero");
    }
    ...
    function test_emitsEvents_withNativeETH() public {
        vm.expectEmit(true, true, false, true);
        emit ChannelHub.Withdrawn(address(0), DEPOSIT_AMOUNT);
    ...
    function test_succeeds_fullBalance_nativeETH() public {
        vm.prank(node);
        cHub.withdrawFromNode(recipient, address(0), INITIAL_BALANCE);

        assertEq(recipient.balance, INITIAL_BALANCE, "Recipient should receive full ETH balance");
        assertEq(cHub.getNodeBalance(address(0)), 0, "Node vault balance should be zero");
    }
    ...
    function test_reverts_whenERC20TransferFails_balanceUnchanged() public {
        ...
        uint256 balanceBefore = cHub.getNodeBalance(address(token));

        vm.prank(node);
        vm.expectRevert();
        cHub.withdrawFromNode(recipient, address(token), DEPOSIT_AMOUNT);

        assertEq(cHub.getNodeBalance(address(token)), balanceBefore, "Vault balance must be unchanged on revert");
