...
func TestNewClientValidation(t *testing.T) {
	t.Run("should fail with invalid private key", func(t *testing.T) {
		client, err := NewClient(log.NewNoopLogger(), "not-a-valid-key", "ws://localhost:8080", "usdc", decimal.NewFromInt(10), 1)
	...
	t.Run("should fail with invalid URL", func(t *testing.T) {
		...
		client, err := NewClient(log.NewNoopLogger(), validKey, "ws://localhost:19999", "usdc", decimal.NewFromInt(10), 1)
	...
	t.Run("should handle 0x prefixed key", func(t *testing.T) {
		...
		client, err := NewClient(log.NewNoopLogger(), key, "ws://localhost:19999", "usdc", decimal.NewFromInt(10), 1)
		if err == nil {
