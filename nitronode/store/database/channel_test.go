...
func TestDBStore_GetHomeChannelStatus(t *testing.T) {
	t.Run("Returns status for existing home channel", func(t *testing.T) {
		db, cleanup := SetupTestDB(t)
		defer cleanup()
		...
		status, err := store.GetHomeChannelStatus(homeChannelID)
		require.NoError(t, err)
		require.NotNil(t, status)
		assert.Equal(t, core.ChannelStatusChallenged, *status)
	})

	t.Run("Returns nil when channel does not exist", func(t *testing.T) {
		db, cleanup := SetupTestDB(t)
		defer cleanup()

		store := NewDBStore(db)
		status, err := store.GetHomeChannelStatus("0xmissing")
		require.NoError(t, err)
		assert.Nil(t, status)
	})

	t.Run("Returns nil for non-home channel", func(t *testing.T) {
		db, cleanup := SetupTestDB(t)
		defer cleanup()
		...
		status, err := store.GetHomeChannelStatus(escrowID)
		require.NoError(t, err)
		assert.Nil(t, status)
	})

	t.Run("Accepts channelID without 0x prefix", func(t *testing.T) {
		db, cleanup := SetupTestDB(t)
		defer cleanup()

		store := NewDBStore(db)
		require.NoError(t, store.CreateChannel(core.Channel{
			ChannelID:         "0xhomenoprefix",
			UserWallet:        "0xuser123",
			Asset:             "usdc",
			Type:              core.ChannelTypeHome,
			BlockchainID:      1,
		...
		status, err := store.GetHomeChannelStatus("homenoprefix")
		require.NoError(t, err)
		require.NotNil(t, status)
