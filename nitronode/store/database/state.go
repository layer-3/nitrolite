...
// rows whose home_channel_id matches channelID, whose epoch matches the supplied epoch,
// whose version is strictly greater than minVersion, whose node_sig is NULL, and whose
// transition type is one of the receiver-credit kinds (transfer_receive, release). The
// epoch filter is defensive: channel IDs are unique per nonce so cross-epoch contamination
// shouldn't occur, but pinning the epoch makes the invariant explicit. Used to compute
// the squashed credit for a ChallengeRescue when a challenged channel is closed onchain.
func (s *DBStore) SumUnsignedReceiverStateAmountsAfterVersion(channelID string, minVersion, epoch uint64) (decimal.Decimal, error) {
	cid := strings.ToLower(channelID)
	var row struct {
	...
	`, cid, epoch, minVersion, uint8(core.TransitionTypeTransferReceive), uint8(core.TransitionTypeRelease)).Scan(&row).Error
	if err != nil {
