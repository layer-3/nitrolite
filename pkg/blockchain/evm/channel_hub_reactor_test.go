...
// newReactor creates a ChannelHubReactor wired to the provided mocks. The
// reactor's read-only ChannelHub view is nil here because the mock event
// handler never reads it — the unit tests in this file assert dispatch only.
func newReactor(blockchainID uint64, nodeAddress string, handler *mockChannelHubEventHandler, assetStore *MockAssetStore, store *mockChannelHubStore) *ChannelHubReactor {
	...
	return NewChannelHubReactor(blockchainID, nodeAddress, handler, assetStore, useStoreInTx, nil)
}
