...
| What we have | Status |
|---|---|
| Unified MCP server (`sdk/mcp/`) | Works locally, 33 resources, 8 tools, 3 prompts — covers TypeScript and Go SDKs |
| Protocol docs, clearnode API, SDK source | Indexed at startup by the MCP server |

**The problem:** The server reads files from the repo at runtime (`docs/protocol/`, `clearnode/docs/`, `sdk/ts/src/`, `sdk/go/`). If you don't have the full repo cloned, it doesn't work. No external developer can use it.
