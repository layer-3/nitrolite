...
describe('compat public runtime API drift guard', () => {
    ...
    it('proves adversarial public signature changes are observable', () => {
        ...
        const mutatedClient = mutated.find((member) => member.name === 'NitroliteClient');
    ...
    it('proves adversarial type-only export removal is observable', () => {
        const api = serializePublicApi();
        expect(api.some((member) => member.name === 'NitroliteClientConfig' && member.kind === 'interface')).toBe(true);

        const mutated = api.filter((member) => member.name !== 'NitroliteClientConfig');
        expect(mutated.some((member) => member.name === 'NitroliteClientConfig')).toBe(false);
    });

    it('proves adversarial function parameter changes are observable', () => {
        const api = serializePublicApi();
        const builder = api.find((member) => member.name === 'buildClientOptions');
        const original = builder?.signatures?.[0] ?? '';
        expect(original).toContain('config: CompatClientConfig');

        const mutated = original.replace('config: CompatClientConfig', 'config: unknown');
        expect(mutated).not.toEqual(original);
    });

    it('proves adversarial enum value changes are observable', () => {
        const api = serializePublicApi();
        const method = api.find((member) => member.name === 'RPCMethod');
        const original = method?.members?.join('|') ?? '';
        expect(original).toContain('Ping');

        const mutated = original.replace('Ping', 'PingChanged');
        expect(mutated).not.toEqual(original);
    });
